...
class StreakService {
  ...
  async getUserStreak(userId: string): Promise<any> {
    // 检查用户是否有学习连续性记录
    const { data: streakData, error: streakError } = await supabaseService.getClient()
      .from('user_streaks')
      .select('*')
      .eq('user_id', userId)
      .maybeSingle();
    ...
    if (!streakData) {
      // 如果没有记录，创建一个新记录
      const { data: newStreakData, error: newStreakError } = await supabaseService.getClient()
        .from('user_streaks')
        .insert([
          {
            user_id: userId,
  ...
  async updateUserStreak(userId: string): Promise<any> {
    ...
    // 更新数据库
    const { data: updatedStreakData, error: updateError } = await supabaseService.getClient()
      .from('user_streaks')
      .update({
  ...
  async getStreakRewards(userId: string): Promise<any[]> {
    ...
    // 获取所有连续学习奖励
    const { data: allRewards, error: rewardsError } = await supabaseService.getClient()
      .from('streak_rewards')
      .select('*')
    ...
    // 获取用户已获得的奖励
    const { data: userRewards, error: userRewardsError } = await supabaseService.getClient()
  ...
  async grantStreakReward(userId: string, rewardId: string): Promise<any> {
    // 检查奖励是否存在
    const { data: reward, error: rewardError } = await supabaseService.getClient()
      .from('streak_rewards')
      .select('*')
      .eq('id', rewardId)
      .single();
    ...
    // 检查用户是否已获得该奖励
    const { data: existingReward, error: existingRewardError } = await supabaseService.getClient()
      .from('user_streak_rewards')
      .select('*')
      .eq('user_id', userId)
    ...
    // 授予奖励
    const { data: grantedReward, error: grantError } = await supabaseService.getClient()
      .from('user_streak_rewards')
      .insert([
        {
          user_id: userId,
