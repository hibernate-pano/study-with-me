...
ReactDOM.render(
  ...
  document.getElementById('root')
      ...
      explanation:
        '这是一个最简单的React组件示例。我们定义了一个名为HelloWorld的函数组件，它返回一个h1元素。然后使用ReactDOM.render()将组件渲染到DOM中id为"root"的元素内。',
    },
  ],
  exercises: [
    {
  ...
  faq: [
    {
      ...
      answer:
        "React严格来说是一个库，而不是框架。它专注于视图层，负责渲染UI组件，通常需要与其他库（如React Router用于路由，Redux用于状态管理）结合使用来构建完整的应用程序。",
    },
    {
      question: "学习React需要什么前置知识？",
      answer:
        "学习React之前，你应该具备HTML、CSS和JavaScript的基础知识。特别是对JavaScript ES6+特性（如箭头函数、解构赋值、类）的理解会很有帮助。",
    },
    {
      question: "React适合初学者吗？",
      answer:
        "对于已经掌握JavaScript基础的开发者来说，React是一个相对容易上手的库。它的核心概念不多，文档完善，社区资源丰富，这些都有助于初学者学习。",
    },
  ],
};
...
const mockExercises = [
  {
    ...
    answer: "组件化开发和虚拟DOM",
    explanation:
      "React的两个核心优势是组件化开发方式和虚拟DOM技术。组件化使代码更易于维护和复用，而虚拟DOM通过最小化实际DOM操作提高了性能。",
  },
  {
    id: 2,
    ...
    explanation:
      "React是由Facebook(现在的Meta)开发的，并于2013年开源。它最初是为了解决Facebook在构建大型应用时面临的挑战而创建的。",
  },
  {
    id: 3,
    ...
    explanation:
      "React采用单向数据流，使数据流动更可预测，而Angular使用双向数据绑定，可以自动同步模型和视图。此外，React是一个库，专注于UI，而Angular是一个完整的框架。",
  },
];
  ...
  useEffect(() => {
    const fetchChapterContent = async () => {
      ...
      try {
        ...
        try {
          // 首先尝试直接获取章节内容
          const contentResponse = await contentApi.getById(
          ...
          <Box sx={{ mb: 4 }}>
            ...
            >
              <Typography variant="body2" color="text.secondary">
                {Object.values(loadingSteps).find(
        ...
        <Box component="main" sx={{ flexGrow: 1, p: 3 }}>
          <Container maxWidth="lg" sx={{ mt: 4, mb: 4 }}>
            ...
            <Box>
              ...
              {/* 学习内容标签页 - 使用新的ContentDisplay组件 */}
              <TabPanel value={tabValue} index={0}>
                <ContentDisplay
                  title={chapterContent?.title}
                  summary={chapterContent?.summary}
                  concepts={chapterContent?.concepts || []}
                  codeExamples={chapterContent?.codeExamples || []}
                  faq={chapterContent?.faq || []}
              ...
              <TabPanel value={tabValue} index={1}>
                <Paper sx={{ p: 3, mb: 3, borderRadius: 2 }}>
                  <Box sx={{ display: "flex", alignItems: "center", mb: 3 }}>
                    ...
                    <Typography variant="h5">章节练习</Typography>
                  </Box>
