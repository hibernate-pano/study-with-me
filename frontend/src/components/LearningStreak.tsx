...
export default function LearningStreak() {
  ...
  useEffect(() => {
    const fetchStreakData = async () => {
      ...
      try {
        // 获取用户连续学习数据
        const streakResponse = await streaksApi.getUserStreak(user.id);
  ...
  const getNextRewardTarget = () => {
    ...
    // 按所需天数排序
    const sortedRewards = [...availableRewards].sort((a, b) => a.days_required - b.days_required);
          ...
          <Grid item xs={12} md={6}>
            <Card sx={{ height: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', p: 3 }}>
              ...
              <Box sx={{ position: 'relative', display: 'inline-flex', my: 2 }}>
                <CircularProgress
                  variant="determinate"
                  value={nextReward ? (streakData.current_streak / nextReward.days_required) * 100 : 100}
                  size={120}
                  thickness={5}
                  color="primary"
                />
                ...
                {nextReward ? (
                  <Box>
                    <Typography variant="body2" gutterBottom>
                      再坚持 {nextReward.days_required - streakData.current_streak} 天可获得:
                    </Typography>
                    <Box sx={{ mb: 2 }}>
                      <LinearProgress 
                        variant="determinate" 
                        value={(streakData.current_streak / nextReward.days_required) * 100} 
                        sx={{ height: 10, borderRadius: 5, mb: 1 }}
                      />
                      <Typography variant="body2" color="text.secondary" align="right">
                        {streakData.current_streak}/{nextReward.days_required} 天
                      </Typography>
                    </Box>
                    <Card 
                      variant="outlined" 
          ...
          <>
            ...
            <DialogContent>
              <Box sx={{ display: 'flex', alignItems: 'center', mb: 2 }}>
                <Box sx={{ mr: 2, p: 1, bgcolor: 'primary.main', borderRadius: '50%', color: 'white' }}>
            ...
            <DialogActions>
              <Button onClick={handleCloseRewardDialog}>
                关闭
              </Button>
              {streakData.current_streak >= selectedReward.days_required && (
                <Button 
